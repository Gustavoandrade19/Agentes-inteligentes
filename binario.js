/*
Mapeamento Teórico (O Agente Binário)
• Ambiente: Uma lista (array) de dados numéricos obrigatoriamente ordenada.
• Percepção (Sensores): O agente monitoriza os limites operacionais (baixo, alto) e calcula o índice do meio para ler o valor contido naquela posição.
• Estado Interno (Memória): O agente armazena e restringe o intervalo do espaço de busca a cada iteração.
• Regra Racional (Lógica Interna): 
  1. Se valor_meio == alvo -> Sucesso (Retorna posição).
  2. Se valor_meio > alvo -> Reduz o limite superior (alto = meio - 1).
  3. Se valor_meio < alvo -> Eleva o limite inferior (baixo = meio + 1).
*/
